...
export default function ReconcileQueue() {
  ...
  const handleDecision = async (action) => {
    ...
    try {
      ...
      // Reset generated code if rejected
      if (action === "REJECT") {
        setGeneratedNationalCode(null);
      }
    } catch (err) {
    ...
    <div className="max-w-5xl mx-auto space-y-6 text-slate-800">
      ...
      {adjudicationResult && !isProcessing && (
        <div className="space-y-6">
          ...
          {/* ========================================================= */}
          {/* STEP 1: MANDATORY ADJUDICATION DECISION PANEL             */}
          {/* ========================================================= */}
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Please approve or reject the reconciliation before a National Code can be issued.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2.5 w-full sm:w-auto">
              <button
                ...
                className={`flex-1 sm:flex-none px-4 py-2 text-xs font-semibold rounded-lg border transition disabled:opacity-50 cursor-pointer ${
          ...
          {/* ========================================================= */}
          {/* STEP 2: OUTPUT REJECTED NOTIFICATION                      */}
          {/* ========================================================= */}
          {decisionStatus === "REJECT" && (
            <div className="bg-rose-50 border border-rose-200 rounded-xl p-5 shadow-sm flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-xl">🛑</span>
                <div>
                  <h4 className="text-sm font-bold text-rose-900">
                    Entry Rejected by Reviewer
                  </h4>
                  <p className="text-xs text-rose-700 mt-0.5">
                    This material reconciliation was rejected. National Material Master Code generation is disabled.
                  </p>
                </div>
              </div>
              <span className="text-xs font-bold font-mono px-3 py-1 rounded bg-white border border-rose-300 text-rose-700">
                NO CODE ASSIGNED
              </span>
            </div>
          )}
          ...
          {/* ========================================================= */}
          {/* STEP 3: NATION CODE ONLY REVEALED ON APPROVE              */}
          {/* ========================================================= */}
          {decisionStatus === "APPROVE" && (
            <>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        100% Deterministic match. Approved for Standard National Material Master Code generation.
                      </p>
                    </div>
                    ...
                    {!generatedNationalCode ? (
                      <button
                        ...
                        className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs rounded-lg shadow-sm transition flex items-center gap-2 cursor-pointer"
                      >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-2 text-xs font-semibold rounded-lg border border-slate-300 bg-slate-50 hover:bg-slate-100 text-slate-700 cursor-pointer"
                        >
                          {copied ? "Copied! ✓" : "Copy"}
                        </button>
                      </div>
                ...
                <div className="bg-amber-50 rounded-xl border border-amber-200 p-6 shadow-sm">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-semibold text-xs rounded-lg shadow-sm transition cursor-pointer"
                    >
                      Upload Data Sheet
                    </button>
                  </div>
                ...
                <div className="bg-white rounded-xl border border-blue-200 p-6 shadow-sm">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Approved as functionally interchangeable under API 6D. Ready for Equivalence Code assignment.
                      </p>
                    </div>
                    ...
                    {!generatedNationalCode ? (
                      <button
                        onClick={handleGetNationalCode}
                        disabled={isGeneratingCode}
                        className="px-5 py-2.5 bg-blue-700 hover:bg-blue-800 text-white font-semibold text-xs rounded-lg shadow-sm transition flex items-center gap-2 cursor-pointer"
                      >
